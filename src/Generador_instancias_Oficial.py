...
def generar_instancia_irp(
    ...
    for i in range(num_clientes):
        ...
        for j in range(num_productos):
            ...
            inv.text = str(10) # inventario inicial
...
def generar_lote_instancias(lista_configuraciones, num_repeticiones=5, semilla_base=1):
    ...
    for i, config_nombre in enumerate(lista_configuraciones):
        ...
        for j in range(num_repeticiones):
            ...
            # 2. Se crea un NOMBRE DE ARCHIVO ÚNICO que incluye el número de la repetición.
            nombre_archivo_final = f"instancia_{config_nombre}_rep{j+1}.xml"
            print(f"  Generando: {nombre_archivo_final} con semilla {semilla_actual}")

            # 3. Se llama a la función generadora original con los parámetros de esta repetición.
            generar_instancia_irp(
                ancho_zona=1000,
                largo_zona=1000,
                horizonte_tiempo=100,
                num_productos=P,
                num_clientes=N,
                num_vehiculos=M,
                semilla=semilla_actual,
                carpeta_instancias="INSTANCIAS_GENERADAS_chicas",
