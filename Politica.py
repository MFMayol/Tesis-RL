...
class RollOutSimple(PoliticasSimples):
    ...
    def tomar_accion(self, estado: Estado):
        ...
        for accion in acciones_factibles:
            ...
            recompensas_totales = [self.simular_episodio_rollout(accion, estado) for _ in range(2)] # Simulamos 2 veces el futuro con esa acción
...
class MonteCarlo(Modelos_de_aproximacion):
    ...
    def __init__(self, instancia, proceso, episodios, epsilon, learning_rate):
        ...
        self.tiempo_entrenamiento = None # aquí se registrará el tiempo que le toma entrenarse
    ...
    def entrenar_modelo(self):
        ...
        for episodio in range(self.episodios):
            trayectoria = self.ejecutar_politica_epsilon_greedy() # devuelve una lista de diccionarios con el estado, accion, recompensa, features
    ...
    def entrenar_modelo(self):
        ...
        for episodio in range(self.episodios):
            ...
            while True:
                ...
                G = recompensa + self.gamma * nuevo_Q 
            ...
            if episodio % 100 == 0:
                self.actualizar_mejores_betas()
                print(f"Episodio: {episodio} ok" )

        fin = time.time()  #  Termina de medir tiempo
        self.tiempo_entrenamiento = round(fin - inicio)


class Fourier(MonteCarlo):
...
class Qlearning_Fourier(Fourier):
    def __init__(self, instancia, proceso, episodios, epsilon, learning_rate, max_i, gamma):
        super().__init__(instancia, proceso, episodios, epsilon, learning_rate, max_i)
        self.gamma = gamma

    def entrenar_modelo(self):
        ''' Método que ejecuta el algoritmo Q-learning con fourier'''
        self.crear_betas()
...
class Qlearning_buffer(Qlearning):
    '''
    Clase que implementa Q-learning con función de valor aproximada
    y Replay Buffer que almacena features(s,a) para mejorar eficiencia.
    '''

    def __init__(self, instancia, proceso, episodios, gamma, epsilon, learning_rate):
        super().__init__(instancia, proceso, episodios, gamma, epsilon, learning_rate)
        self.buffer = deque(maxlen=10_000)  # replay buffer con límite de experiencias
        self.batch_size = 2
        self.start_training_after = 200  # para evitar entrenar con buffer vacío

    def entrenar_modelo(self):
        '''Entrenamiento Q-learning con Replay Buffer'''
        self.crear_betas()
        self.inicializar_mejores_betas()
        proceso = self.proceso 
        es_terminal = self.es_terminal
        ...
        for episodio in range(self.episodios):
            ...
            while True:
                ...
                # Almacenamos (estado, accion, features, recompensa, nuevo_estado, done)
                self.buffer.append((estado, accion, features, recompensa, nuevo_estado, done))
    ...
    def actualizar_desde_buffer(self):
        '''Actualiza los betas con muestras desde el replay buffer'''
        batch = random.sample(self.buffer, self.batch_size)

        for estado, accion, features, recompensa, nuevo_estado, done in batch:
            if done:
                G = recompensa  # no hay futuro si es terminal
            else:
                mejor_accion_nuevo_estado = self.politica_optima(nuevo_estado, self.obtener_acciones(nuevo_estado)) # fijo
                features_nuevo = self.obtener_features(nuevo_estado, mejor_accion_nuevo_estado)
                Q_nuevo = np.dot(features_nuevo, self.betas) # agregar fijo 
                G = recompensa + self.gamma * Q_nuevo

            # Actualizar pesos con SGD usando features ya almacenados
            self.SGD(features, G)